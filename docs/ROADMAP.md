...
- [ ] 업데이트 직전 서비스/프로세스 상태 스냅샷 저장
- [ ] Apache/PHP 구성요소 폴더 자동 백업
- [ ] MariaDB 설정 + data 물리 백업
- [ ] MariaDB 논리 백업 실행 방식 결정 및 구현
- [ ] 백업 manifest(JSON) 저장
- [ ] 파일별 크기/SHA256/원본 경로 기록
- [ ] 롤백에 필요한 서비스 상태와 대상 버전 기록

### 3C — 패키지 준비와 비교

- [ ] 선택 버전 실제 패키지 URL 확정 및 다운로드
- [ ] 가능한 경우 SHA256/PGP 검증
- [ ] ZIP 내부 아키텍처/필수 실행 파일/모듈 구조 검사
- [ ] 기존 설치와 신규 패키지 파일 구조 비교
- [ ] 기존 설정과 신규 기본 설정 diff 생성
- [ ] 자동 병합 가능 / 사용자 확인 필요 / 폐기 후보 분류
- [ ] Apache 외부 모듈 및 PHP extension 호환성 비교
- [ ] MariaDB 목표 버전별 중간 업그레이드 경로 확정
