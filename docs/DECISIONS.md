...
## D-017 목표 버전 선택은 현재 계열로 제한하지 않는다

사용자가 선택할 수 있는 목표 버전은 현재 major/minor 계열로 제한하지 않는다. 현재 버전보다 높은 공식 릴리스를 가능한 범위에서 모두 카탈로그에 포함하고, 최신 버전도 동일한 선택지로 취급한다.

버전 목록 공급원은 다음과 같이 분리한다.

- Apache: ASF 공식 archive에서 2.4 릴리스 목록을 확보한다. ASF가 Windows 바이너리를 직접 제공하지 않으므로 Windows ZIP이 자동 해결되지 않는 버전은 패키지 직접 지정 흐름으로 연결한다.
- PHP: PHP Windows 공식 archive에서 현재 아키텍처와 Apache 연동 방식에 맞는 TS/NTS ZIP을 버전별로 확보하고 실제 ZIP URL을 연결한다.
- MariaDB: MariaDB 공식 DLM에서 현재 계열 이상 각 major/minor 및 패치 릴리스를 수집하고 공식 winx64 패키지 페이지를 연결한다. EOL 계열은 선택 가능하되 명시적으로 표시한다.

업데이트 버전과 패키지 공급원을 분리하므로 자동 패키지 탐색이 실패해도 버전 선택과 업데이트 계획 계산은 유지된다. 사용자가 지정한 ZIP은 Phase 3에서 내부 버전, PE 아키텍처, 필수 실행 파일/모듈 구조를 다시 검사한 후에만 실제 업데이트 단계로 넘긴다.
