...
- 최신 버전과 각 major.minor 계열의 최신 패치 버전을 목표 버전으로 선택
- 선택한 목표 버전까지 필요한 업데이트 작업 경로 계산

버전 선택 목록은 모든 패치 버전을 나열하지 않습니다. 예를 들어 PHP는 `8.5.x 최신`, `8.4.x 최신`, `8.3.x 최신`처럼 각 major.minor 계열의 최신 패치 버전만 보여줍니다. 최신 전체 버전은 별도로 식별됩니다.

완전 자동 업데이트가 어려운 경우에도 기능을 단순 차단하지 않습니다. 향후 단계에서 자동 백업, 기존/신규 설정 비교, 충돌 항목 선택, 적용 후 검증과 롤백을 조합한 **보조 업데이트 흐름**을 제공합니다.
