...
public sealed class VerifiedPhpUpdateExecutor : IPhpUpdateExecutor
{
    ...
    {
        UpdateProgressReporter.Report(XamppComponentType.Php, "BackupVerify", "롤백 백업 무결성을 확인하는 중...", 10);
        BackupIntegrityVerifier.Verify(backup);
        UpdateProgressReporter.Report(XamppComponentType.Php, "BackupVerify", "롤백 백업 무결성 확인 완료", 20);

        var snapshotWarnings = new List<string>();
        UpdateProgressReporter.Report(XamppComponentType.Php, "BeforeSnapshot", "업데이트 전 설정 snapshot을 저장하는 중...", 25);
        try
        {
        ...
        UpdateProgressReporter.Report(XamppComponentType.Php, "Execute", "PHP 교체·설정 마이그레이션·런타임 검증을 진행하는 중...", 35);
        ...
        {
            UpdateProgressReporter.Report(XamppComponentType.Php, "AfterSnapshot", "업데이트 후 설정 snapshot을 저장하는 중...", 90);
            try
            {
            ...
            UpdateProgressReporter.Report(XamppComponentType.Php, "Completed", $"PHP {target.Version} 업데이트 완료", 100);
        }
