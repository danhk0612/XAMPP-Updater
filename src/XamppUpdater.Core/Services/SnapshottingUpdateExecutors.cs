...
public sealed class SnapshottingMariaDbUpdateExecutor : IMariaDbUpdateExecutor
{
    ...
    {
        UpdateProgressReporter.Report(XamppComponentType.MariaDb, UpdateProgressStages.BackupVerify, "논리/물리 롤백 백업 무결성을 확인하는 중...", 10);
        BackupIntegrityVerifier.Verify(backup, requireLogicalBackup: true);
        UpdateProgressReporter.Report(XamppComponentType.MariaDb, UpdateProgressStages.BackupVerify, "MariaDB 롤백 백업 무결성 확인 완료", 20);

        var warnings = new List<string>();
        UpdateProgressReporter.Report(XamppComponentType.MariaDb, UpdateProgressStages.BeforeSnapshot, "업데이트 전 MariaDB 설정 snapshot을 저장하는 중...", 25);
        try
        {
        ...
        UpdateProgressReporter.Report(XamppComponentType.MariaDb, UpdateProgressStages.Execute, "MariaDB 서비스 중지·바이너리 교체·데이터 복사·upgrade 검증을 진행하는 중...", 35);
        ...
        {
            UpdateProgressReporter.Report(XamppComponentType.MariaDb, UpdateProgressStages.AfterSnapshot, "업데이트 후 MariaDB 설정 snapshot을 저장하는 중...", 90);
            try
            {
            ...
            UpdateProgressReporter.Report(XamppComponentType.MariaDb, UpdateProgressStages.Completed, $"MariaDB {target.Version} 업데이트 완료", 100);
        }
