...
public static class UpdateTargetPlanner
{
        ...
        CandidatePackageCatalog candidates)
    {
        return new UpdateTargetCatalog(
            BuildOptions(XamppComponentType.Apache, installation, online, candidates),
            BuildOptions(XamppComponentType.Php, installation, online, candidates),
            BuildOptions(XamppComponentType.MariaDb, installation, online, candidates));
    }
        ...
        InstallationCompatibilityProfile profile)
    {
        if (!Version.TryParse(currentVersion, out var current) || !Version.TryParse(target.Version, out var next))
        {
            return new UpdatePlan(
                ...
                currentVersion,
                target.Version,
                CandidateCompatibilityStatus.ManualReview,
        ...
        CandidatePackageCatalog candidates)
    {
        ...
        Add(candidate.Version, "현재 계열 추천", UpdateTargetSource.SameSeriesCandidate, false, candidate.DownloadUrl is not null);
        Add(onlineVersion.XamppBundledVersion, "XAMPP 공식 기준", UpdateTargetSource.XamppBundle, false, false);
        Add(onlineVersion.UpstreamLatestVersion, "최신", UpdateTargetSource.UpstreamLatest, true, type == XamppComponentType.Apache ? candidate.Version == onlineVersion.UpstreamLatestVersion : false);
    ...
    private static UpdatePlan BuildApachePlan(string currentVersion, UpdateTargetOption target, InstallationCompatibilityProfile profile)
    {
        var steps = new List<UpdatePlanStep>
        {
            ...
            Auto("패키지 확인", "대상 Windows x64/x86 Apache 패키지와 VC++ 런타임 조건을 확인합니다."),
    ...
    private static UpdatePlan BuildPhpPlan(string currentVersion, UpdateTargetOption target, InstallationCompatibilityProfile profile)
    {
        ...
        var steps = new List<UpdatePlanStep>
        {
            ...
            Auto("대상 패키지 선택", $"현재 환경({CompatibilityEvaluator.FormatArchitecture(profile.PhpArchitecture)}, {(profile.Php.ThreadSafe == true ? "TS" : "NTS")})에 맞는 PHP {target.Version} Windows 패키지를 찾습니다."),
    ...
    private static UpdatePlan BuildMariaDbPlan(string currentVersion, UpdateTargetOption target, InstallationCompatibilityProfile profile)
    {
        ...
        var steps = new List<UpdatePlanStep>
        {
            ...
            Assist("단계별 바이너리 교체", "각 단계의 공식 Windows 패키지를 준비하고 설정/서비스 경로를 유지하며 순차 교체합니다."),
