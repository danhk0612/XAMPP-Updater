...
internal sealed partial class PhpMyAdminUpdateService
{
    ...
    public PhpMyAdminInstallationState Inspect(string xamppRoot)
    {
        ...
        {
            return new PhpMyAdminInstallationState(
                false,
                Path.Combine(Path.GetFullPath(xamppRoot), "phpMyAdmin"),
                null,
                "XAMPP 루트에 phpMyAdmin 폴더가 없습니다.");
        }
        ...
        return new PhpMyAdminInstallationState(
            ...
            directory,
            version,
            version is null ? "phpMyAdmin은 설치되어 있지만 버전을 판별하지 못했습니다." : null);
    }
    ...
    public async Task<PhpMyAdminReleaseInfo> GetLatestAsync(CancellationToken cancellationToken = default)
    {
        ...
        {
            throw new InvalidOperationException("phpMyAdmin 최신 버전 정보를 읽지 못했습니다.");
        }
        ...
        if (root.TryGetProperty("releases", out var releases) && releases.ValueKind == JsonValueKind.Array)
        {
            ...
            {
                if (!string.Equals(release.GetProperty("version").GetString(), version, StringComparison.OrdinalIgnoreCase))
                {
                    continue;
                }

                if (release.TryGetProperty("php_versions", out var phpElement))
                {
                    phpRange = phpElement.GetString();
                }
                if (release.TryGetProperty("mysql_versions", out var dbElement))
                {
                    databaseRange = dbElement.GetString();
                }
                break;
            }
        }
        ...
        return new PhpMyAdminReleaseInfo(
            version,
            releaseDate,
            baseUrl,
            baseUrl + ".sha256",
    ...
    public PhpMyAdminCompatibility EvaluateCompatibility(
        ...
        string? databaseVersion)
    {
        ...
        if (release.PhpVersionRange?.Contains('<') == true &&
            TryGetExclusiveUpperBound(release.PhpVersionRange, out var upper) &&
            TryParseVersion(phpVersion, out php) && php >= upper)
        {
        ...
        var summary = canUpdate
            ? warnings.Count == 0
                ? "현재 PHP/DB 버전에서 최신 안정판 업데이트를 진행할 수 있습니다."
                : "업데이트 가능하지만 호환성 주의사항이 있습니다."
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        {
            throw new InvalidOperationException("현재 XAMPP에 phpMyAdmin이 설치되어 있지 않습니다.");
        }
        if (string.IsNullOrWhiteSpace(state.Version))
        {
            throw new InvalidOperationException("현재 phpMyAdmin 버전을 확인할 수 없어 안전한 업데이트를 진행할 수 없습니다.");
        }

        var compatibility = EvaluateCompatibility(release, phpVersion, databaseVersion);
        if (!compatibility.CanUpdate)
        {
            throw new InvalidOperationException(string.Join(Environment.NewLine, compatibility.Warnings));
        }

        if (CompareVersions(state.Version, release.Version) >= 0)
        {
            throw new InvalidOperationException($"현재 phpMyAdmin {state.Version}은 최신 안정판 {release.Version} 이상입니다.");
        }
        ...
        var backupPath = Path.Combine(
            backupRoot,
            $"phpMyAdmin-{state.Version}-to-{release.Version}-{DateTime.Now:yyyyMMdd-HHmmss}");
        ...
        try
        {
            ...
            expectedSha = await DownloadChecksumAsync(release.ChecksumUrl, cancellationToken);
            ...
            if (Directory.Exists(oldSwapPath))
            {
                Directory.Delete(oldSwapPath, recursive: true);
            }
            swapped = false;

            Report(progress, "Completed", 100, $"phpMyAdmin {state.Version} → {release.Version} 업데이트 완료");
            return new PhpMyAdminUpdateResult(state.Version, release.Version, backupPath, actualSha);
        }
        catch
        {
            ...
            if (swapped)
            {
                try
                {
                    ...
                    {
                        Directory.Delete(state.DirectoryPath, recursive: true);
                    }
                    if (Directory.Exists(oldSwapPath))
                    {
                        Directory.Move(oldSwapPath, state.DirectoryPath);
                    }
                }
                catch
                {
                    // 원래 예외를 유지한다. persistent backupPath는 수동 복구용으로 남긴다.
                }
            }
    ...
    private static async Task DownloadFileAsync(
        string url,
        string destination,
        IProgress<PhpMyAdminUpdateProgress>? progress,
        CancellationToken cancellationToken)
    {
        ...
        while (true)
        {
            ...
            var percent = total is > 0
                ? 5 + (int)Math.Min(24, received * 24 / total.Value)
                : 15;
    ...
    private static async Task<string> DownloadChecksumAsync(string url, CancellationToken cancellationToken)
    {
        ...
        if (!match.Success)
        {
            throw new InvalidDataException("phpMyAdmin SHA256 응답에서 해시를 찾지 못했습니다.");
        }
    ...
    private static string FindPayloadRoot(string stagingRoot)
    {
        ...
        var candidates = Directory.EnumerateDirectories(stagingRoot)
            .Where(path => File.Exists(Path.Combine(path, "index.php")))
            .ToArray();
        if (candidates.Length != 1)
        {
            throw new InvalidDataException("phpMyAdmin ZIP 내부의 설치 루트를 하나로 확정하지 못했습니다.");
        }
        return candidates[0];
    }
    ...
    private static void ValidatePayload(string payloadRoot, string expectedVersion)
    {
        var required = new[]
        {
            Path.Combine(payloadRoot, "index.php"),
            Path.Combine(payloadRoot, "libraries"),
            Path.Combine(payloadRoot, "vendor")
        };
        foreach (var path in required)
        {
            if (!File.Exists(path) && !Directory.Exists(path))
            {
                throw new InvalidDataException($"phpMyAdmin 필수 파일/폴더가 없습니다: {Path.GetFileName(path)}");
            }
        }
    ...
    private static void PreserveUserFiles(string currentRoot, string newRoot)
    {
        foreach (var fileName in new[] { "config.inc.php", ".htaccess" })
        {
            ...
            {
                File.Copy(source, Path.Combine(newRoot, fileName), overwrite: true);
            }
        }
        ...
        foreach (var directoryName in new[] { "upload", "save" })
        {
            ...
            {
                CopyDirectory(source, Path.Combine(newRoot, directoryName));
            }
        }
    }
    ...
    private static void ValidatePhpConfigSyntax(string xamppRoot, string phpMyAdminRoot)
    {
        ...
        {
            throw new InvalidDataException("기존 phpMyAdmin config.inc.php가 현재 PHP 구문 검사를 통과하지 못했습니다.\n" + stdout + stderr);
        }
    }
    ...
    private static void Report(IProgress<PhpMyAdminUpdateProgress>? progress, string stage, int percent, string message) =>
        progress?.Report(new PhpMyAdminUpdateProgress(stage, percent, message));
